...
public function main() returns error? {
    ...
    int response = check mongoClient->update(replaceDoc, collection, (), replaceFilter, true);
    if (response > 0 ) {
    ...
    int response2 = check mongoClient->update(replaceDoc2, collection, (), replaceFilter2, true);
    if (response2 > 0 ) {
